...
static void glfw_mouse_press(GLFWwindow* window, int button, int action, int modifier)
{
  ...
  if (action == GLFW_PRESS)
  {
	  ...
	  if (id == -1)
	  {
		  ...
		  scn->selected_data_index = (id+1)%2;
		  scn->data().set_colors(Eigen::RowVector3d(0.8, 0.8, 0));
		  std::cout << "found " << i - 1 << std::endl;
		  scn->selected_data_index = id;
		  scn->data().set_colors(Eigen::RowVector3d(0.5, 0, 0));
		  rndr->world = false;
	  }
...
static void glfw_mouse_scroll(GLFWwindow* window, double x, double y)
{
	Renderer* rndr = (Renderer*)glfwGetWindowUserPointer(window);
	if (rndr->world) {
		rndr->GetScene()->MyTranslate(Eigen::Vector3f(0, 0, y * 0.05));
	}
	else {
		rndr->GetScene()->data().MyTranslate(Eigen::Vector3f(0, 0, y * 0.01));
	}
...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int modifier)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		case 'r':
			...
			scn->data().reset();
			scn->data().MyTranslate(Eigen::Vector3f(-0.5, 0, 0));
			scn->selected_data_index = 1;
			scn->data().reset();
			scn->data().MyTranslate(Eigen::Vector3f(0.5, 0, 0));
			scn->selected_data_index = savedIndx;
			scn->reset();
			scn->MyTranslate(Eigen::Vector3f(0, 0, -1));
		...
		case GLFW_KEY_UP:
		{
			...
			if (rndr->world)
				scn->MyRotate(Eigen::Vector3f(1, 0, 0), 0.3f);
			else
				scn->data().MyRotate(Eigen::Vector3f(1, 0, 0), 0.3f);
			break;
		}
		case GLFW_KEY_DOWN:
		{
		...
		{
			if (rndr->world)
				scn->MyRotate(Eigen::Vector3f(0, -1, 0), 0.3f);
			else
				scn->data().MyRotate(Eigen::Vector3f(0, -1, 0), 0.3f);
			break;
		}
		case GLFW_KEY_RIGHT:
		{
			if (rndr->world)
				scn->MyRotate(Eigen::Vector3f(0, 1, 0), 0.3f);
			else scn->data().MyRotate(Eigen::Vector3f(0, 1, 0), 0.3f);
			break;
		}
