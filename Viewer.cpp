...
namespace glfw
{
 ...
 IGL_INLINE void Viewer::animate() {
	 if(isMoving) {
		 if (check_inter_rec(data_list[0].lastTree, data_list[1].lastTree, MakeTrans() * data_list[0].MakeTrans(), MakeTrans() * data_list[1].MakeTrans())) {
			 isMoving = false;
			 std::cout << "collicion" << std::endl;
			 return;
		 }
		 if (selected_data_index == 0) {
			 data().MyTranslate(Eigen::Vector3f(0.01, 0, 0));
		 }
		 else {
			 data().MyTranslate(Eigen::Vector3f(-0.01, 0, 0));
		 }
 ...
 bool   Viewer::check_inter_rec(igl::AABB<Eigen::MatrixXd, 3> t1, igl::AABB<Eigen::MatrixXd, 3> t2, Eigen::Matrix4f A, Eigen::Matrix4f B)
 {
	 ...
	 {
		 data_list[0].lastTree = data_list[0].tree;
		 data_list[1].lastTree = data_list[1].tree;
		 addBox(t1.m_box, t2.m_box, Eigen::RowVector3d(1,1,1));
		 return f;
	 }
	 if (t1.is_leaf())
	 {
